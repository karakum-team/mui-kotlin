...
private fun componentInterface(
    ...
    val body = source
        ...
        .joinToString("\n") { line ->
            ...
            // identifier (Kotlin type generated by us in the same package — e.g. `IconButtonProps`,
            // `SvgIconProps`) — emit it directly instead of dropping to `react.Props`.
                ?: typeSource.takeIf { it.matches(Regex("""\w+Props""")) }?.let { "$it?" }
