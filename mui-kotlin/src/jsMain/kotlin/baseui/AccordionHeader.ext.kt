...
/**
 * The state-dependent arm of `AccordionHeaderProps.className`, upstream
 * `string | ((state: AccordionHeaderState) => string | undefined)`.
 *
 * The prop itself is `Any?`: it is inherited through [BaseUiH3Props] from a parent shared by every part that
 ...
 */
fun AccordionHeaderProps.className(
    block: (state: AccordionHeaderState) -> ClassName?,
) {
    className = block
}

/**
 * The state-dependent arm of `AccordionHeaderProps.style`, upstream
 * `CSSProperties | ((state: AccordionHeaderState) => CSSProperties | undefined)`. See [AccordionHeaderProps.className].
 */
fun AccordionHeaderProps.style(
    block: (state: AccordionHeaderState) -> CSSProperties?,
) {
    style = block
}

/**
 * The callback arm of `AccordionHeaderProps.render`, upstream
 * `ReactElement | ((props: HTMLProps, state: AccordionHeaderState) => ReactElement)`.
 *
 ...
 */
fun AccordionHeaderProps.render(
    block: (props: HTMLAttributes<HTMLElement>, state: AccordionHeaderState) -> ReactElement<*>,
) {
    render = block
}
