...
/**
 * The state-dependent arm of `MenuLinkItemProps.className`, upstream
 * `string | ((state: MenuLinkItemState) => string | undefined)`.
 *
 * The prop itself is `Any?`: it is inherited through [BaseUiAProps] from a parent shared by every part that
 ...
 */
fun MenuLinkItemProps.className(
    block: (state: MenuLinkItemState) -> ClassName?,
) {
    className = block
}

/**
 * The state-dependent arm of `MenuLinkItemProps.style`, upstream
 * `CSSProperties | ((state: MenuLinkItemState) => CSSProperties | undefined)`. See [MenuLinkItemProps.className].
 */
fun MenuLinkItemProps.style(
    block: (state: MenuLinkItemState) -> CSSProperties?,
) {
    style = block
}

/**
 * The callback arm of `MenuLinkItemProps.render`, upstream
 * `ReactElement | ((props: HTMLProps, state: MenuLinkItemState) => ReactElement)`.
 *
