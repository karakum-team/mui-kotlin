...
/**
 * The state-dependent arm of `SliderValueProps.className`, upstream `string | ((state:
 * SliderValueState) => string | undefined)`.
 *
 * The prop itself is `Any?`: it is inherited through [BaseUiOutputProps] from a parent shared by
 ...
 */
fun SliderValueProps.className(block: (state: SliderValueState) -> ClassName?) {
    className = block
}

/**
 * The state-dependent arm of `SliderValueProps.style`, upstream `CSSProperties | ((state:
 * SliderValueState) => CSSProperties | undefined)`. See [SliderValueProps.className].
 */
fun SliderValueProps.style(block: (state: SliderValueState) -> CSSProperties?) {
    style = block
}

/**
 * The callback arm of `SliderValueProps.render`, upstream `ReactElement | ((props: HTMLProps,
 * state: SliderValueState) => ReactElement)`.
 *
