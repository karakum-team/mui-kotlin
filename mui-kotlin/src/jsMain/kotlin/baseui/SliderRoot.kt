...
external interface SliderRootProps : BaseUiDivProps {
    /**
    ...
    /**
     * The locale used by `Intl.NumberFormat` when formatting the value. Defaults to the user's
     * runtime locale.
     */
    ...
    /**
     * Identifies the form that owns the slider inputs. Useful when the slider is rendered outside
     * the form.
     */
    var form: String?
    ...
    /**
     * How the thumb(s) are aligned relative to `Slider.Control` when the value is at `min` or
     * `max`:
     * - `center`: The center of the thumb is aligned with the control edge
     * - `edge`: The thumb is inset within the control such that its edge is aligned with the
     *   control edge
     * - `edge-client-only`: Same as `edge` but renders after React hydration on the client,
     *   reducing bundle size in return
     *
     * @default 'center'
     */
     ...
     *
     * - `'push'` (default): Thumbs push each other without restoring their previous positions when
     *   dragged back.
    ...
    /**
     * Callback function that is fired when the slider's value changed. You can pull out the new
     * value by accessing `event.target.value` (any).
     *
     * The `eventDetails.reason` indicates what triggered the change:
     *
     * - `'input-change'` when the hidden range input emits a change event (for example, via form
     *   integration)
     ...
     */
    var onValueChange:
        Any? /* (value: Value extends number ? number : Value, eventDetails: SliderRootChangeEventDetails) => void */

    /**
     * Callback function that is fired when a value change is committed. Does not fire if the value
     * did not change, or if the change was canceled. **Warning**: This is a generic event, not a
     * change event.
     *
     ...
     */
    var onValueCommitted:
        Any? /* (value: Value extends number ? number : Value, eventDetails: SliderRootCommitEventDetails) => void */
}
...
external interface SliderRootState : FieldRootState {
    ...
    /**
     * The step increment of the slider when incrementing or decrementing. It will snap to multiples
     * of this value. Decimal values are supported.
     *
     * @default 1
     */
    var step: Number
