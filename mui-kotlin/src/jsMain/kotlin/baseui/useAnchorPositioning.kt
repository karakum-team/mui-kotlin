...
external interface UseAnchorPositioningSharedParameters {
    /**
     * An element to position the popup against. By default, the popup will be positioned against
     * the trigger.
     */
    var anchor:
        Any? /* Element | null | VirtualElement | React.RefObject<Element | null> | (() => Element | VirtualElement | null) */
    ...
    /**
     * Which side of the anchor element to align the popup against. May automatically change to
     * avoid collisions.
     *
     * @default 'bottom'
     */
    ...
    /**
     * Distance between the anchor and the popup in pixels. Also accepts a function that returns the
     * distance to read the dimensions of the anchor and positioner elements, along with its side
     * and alignment.
     *
     * The function takes a `data` object parameter with the following properties:
     * - `data.anchor`: the dimensions of the anchor element with properties `width` and `height`.
     * - `data.positioner`: the dimensions of the positioner element with properties `width` and
     *   `height`.
    ...
    /**
     * Additional offset along the alignment axis in pixels. Also accepts a function that returns
     * the offset to read the dimensions of the anchor and positioner elements, along with its side
     * and alignment.
     *
     * The function takes a `data` object parameter with the following properties:
     * - `data.anchor`: the dimensions of the anchor element with properties `width` and `height`.
    ...
    /**
     * Whether to maintain the popup in the viewport after the anchor element was scrolled out of
     * view.
     *
     * @default false
     */
     ...
     * - `'flip'`: keep the requested side when it fits; otherwise try the opposite side (`top` and
     *   `bottom`, or `left` and `right`).
     * - `'shift'`: never change side; keep the requested side and move the popup within the
     ...
     * - `'shift'`: keep side and requested alignment, then nudge the popup along the alignment axis
     *   to fit.
     * - `'none'`: do not correct alignment-axis overflow.
     *
     * `fallbackAxisSide` controls fallback behavior on the perpendicular axis when the preferred
     * axis cannot fit:
     * - `'start'`: allow perpendicular fallback and try the logical start side first (`top` before
     *   `bottom`, or `left` before `right` in LTR).
     * - `'end'`: allow perpendicular fallback and try the logical end side first (`bottom` before
     *   `top`, or `right` before `left` in LTR).
     * - `'none'`: do not fallback to the perpendicular axis.
     *
     * When `side` is `'shift'`, explicitly setting `align` only supports `'shift'` or `'none'`. If
     * `align` is omitted, it defaults to `'flip'`.
     *
     * @example
     *
     * ```jsx
