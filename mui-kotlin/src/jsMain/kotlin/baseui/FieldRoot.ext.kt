 ...
 */
fun FieldRootProps.className(
    block: (state: FieldRootState) -> ClassName?,
) {
    className = block
}

/**
 * The state-dependent arm of `FieldRootProps.style`, upstream
 * `CSSProperties | ((state: FieldRootState) => CSSProperties | undefined)`. See [FieldRootProps.className].
 */
fun FieldRootProps.style(
    block: (state: FieldRootState) -> CSSProperties?,
) {
    style = block
}

/**
 * The callback arm of `FieldRootProps.render`, upstream
 * `ReactElement | ((props: HTMLProps, state: FieldRootState) => ReactElement)`.
 *
