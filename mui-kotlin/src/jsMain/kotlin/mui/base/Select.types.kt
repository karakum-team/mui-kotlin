...
external interface SelectProps<OptionValue> : SelectOwnProps<OptionValue> {
    var component: Any? /* D */
}

external interface SelectOwnProps<OptionValue> : PropsWithChildren, PropsWithClassName {
    /**
    ...
    /**
     * If `true`, selecting multiple values is allowed. This affects the type of the `value`,
    ...
    /**
     * Callback fired when the component requests to be opened. Use in controlled mode (see
     * listboxOpen).
     */
    var onListboxOpenChange: ((isOpen: Boolean) -> Unit)?

    /**
     * A function used to convert the option label to a string. It's useful when labels are elements
     * and need to be converted to plain text to enable navigation using character keys on a
     * keyboard.
     *
    ...
    var renderValue:
        Any? /* (option: SelectValue<SelectOption<OptionValue>, Multiple>) => React.ReactNode */
    ...
    /**
     * The components used for each slot inside the Select. Either a string to use a HTML element or
     * a component.
     *
     * @default {}
     */
