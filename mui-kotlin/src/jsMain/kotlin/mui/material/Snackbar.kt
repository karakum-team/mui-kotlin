...
external interface SnackbarProps :
    ...
    /**
     * The anchor of the `Snackbar`. On smaller screens, the component grows to occupy all the
     ...
     * `onClose` should then set the state of the `open` prop to hide the Snackbar. This behavior is
     ...
     * add the `key` prop to ensure independent treatment of each message. For instance, use
     * `<Snackbar key={message} />`. Otherwise, messages might update in place, and features like
     * `autoHideDuration` could be affected.
     */
    var key: Any?
    ...
    /**
     * Callback fired when the component requests to be closed. Typically `onClose` is used to set
     * state in the parent component, which is used to control the `Snackbar` `open` prop. The
     * `reason` parameter can optionally be used to control the response to `onClose`, for example
     * ignoring `clickaway`.
     *
     * @param {React.SyntheticEvent<any> | Event} event The event source of the callback.
     * @param {string} reason Can be: `"timeout"` (`autoHideDuration` expired), `"clickaway"`, or
     *   `"escapeKeyDown"`.
     */
     ...
     * `autoHideDuration` prop isn't specified, it does nothing. If `autoHideDuration` prop is
     * specified but `resumeHideDuration` isn't, we default to `autoHideDuration / 2` ms.
     */
...
external interface SnackbarSlotProps : Props {
    ...
    /**
     * TS: SlotProps<React.ElementType<SnackbarContentProps>, SnackbarContentSlotPropsOverrides,
     * SnackbarOwnerState>
     */
    var content: SnackbarContentProps?

    /**
     * TS: SlotComponentProps<typeof ClickAwayListener, SnackbarClickAwayListenerSlotPropsOverrides,
     * SnackbarOwnerState>
     */
     ...
     * SnackbarTransitionSlotPropsOverrides, SnackbarOwnerState>
     */
 ...
 */
@JsName("default") external val Snackbar: FC<SnackbarProps>
