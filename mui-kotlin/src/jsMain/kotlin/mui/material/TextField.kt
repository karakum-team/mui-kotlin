...
external interface BaseTextFieldProps :
    StandardProps, FormControlProps, PropsWithChildren, PropsWithSx {
    /**
    ...
    /**
     * The id of the `input` element. Use this prop to make `label` and `helperText` accessible for
     * screen readers.
     */
    var id: ElementId?
    ...
    /**
     * Render a [`Select`](https://mui.com/material-ui/api/select/) element while passing the Input
     * element to `Select` as `input` parameter. If this option is set you must pass the options of
     * the select as children.
     *
     * @default false
     */
    var select: Boolean?
...
external interface StandardTextFieldProps : BaseTextFieldProps {
     ...
     *
     * @param {object} event The event source of the callback. You can pull out the new value by
...
/**
 * The `TextField` is a convenience wrapper for the most common cases (80%). It cannot be all things
 * to all people, otherwise the API would grow out of control.
 *
 * ## Advanced Configuration
 *
 * It's important to understand that the text field is a simple abstraction on top of the following
 * components:
 *
 ...
 * ```
 *
 * For advanced cases, please look at the source of TextField by clicking on the "Edit this page"
 * button above. Consider either:
 *
 ...
 */
@JsName("default") external val TextField: FC<TextFieldProps>
